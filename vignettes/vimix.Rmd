...
niter=10#length(output$L)-1
N = 200
K = 7
D = 2

alphas <- matrix(0, K, niter)
betas <- array(0, c(D,K,niter))
ms <- array(0, c(D,K,niter))
vs <- array(0, c(D,K,niter))
Ws <- array(0, c(D,K,niter))
cs <- matrix(0, D, niter)
Elnfs <- array(0, c(D,N,K, niter))
Resps<- array(0, c(N,K,niter))
Sus <- array(0, c(D,K,niter))
xbars <- array(0, c(D,K,niter))
Nks <- matrix(0, K, niter)
Ndks <- array(0, c(D,K,niter))

for(i in 1:niter){
    alphas[,i] <- output$model_list[[i]]$alpha   
    betas[,,i] <- output$model_list[[i]]$beta    
    ms[,,i] <- output$model_list[[i]]$m  
    vs[,,i] <- output$model_list[[i]]$v       
    Ws[,,i] <- output$model_list[[i]]$W  
    cs[,i] <- output$model_list[[i]]$c       
    Elnfs[,,,i] <- output$model_list[[i]]$Elnf    
    Resps[,,i] <- output$model_list[[i]]$Resp   
    Sus[,,i] <- output$model_list[[i]]$S       
    xbars[,,i] <- output$model_list[[i]]$xbar    
    Nks[,i] <- output$model_list[[i]]$Nk     
    Ndks[,,i] <- output$model_list[[i]]$Ndk
}


Epz=map_dbl(output$bound_list, 2)
Eqz=map_dbl(output$bound_list, 3) 
Eppi=map_dbl(output$bound_list, 4)  
Eqpi=map_dbl(output$bound_list, 5)
EpGamma=map_dbl(output$bound_list, 6) 
EqGamma=map_dbl(output$bound_list, 7) 
EpDelta=map_dbl(output$bound_list, 8) 
EqDelta=map_dbl(output$bound_list, 9) 
EpMuLambda=map_dbl(output$bound_list, 10) 
EqMuLambda=map_dbl(output$bound_list, 11) 
EpX=map_dbl(output$bound_list, 12)
...
```{r example1-vs-posterior, fig.show='hold', message=FALSE, warning=FALSE, cache=TRUE}
output$model$c
output$model$alpha
output$model$beta
output$model$m
output$model$v
output$model$W
# output$model$Elnf[1:2,1:5,1:3]
head(output$model$Resp)
output$model$S
output$model$xbar
output$model$Nk
output$model$Ndk
