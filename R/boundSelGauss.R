...
boundSelGauss = function(X, model, prior, null){
    ...
    for(k in 1:K){
        ...
        for(d in 1:D){
            ...
            log_Lambda[d,k] <- digamma(v[d,k]/2) + log(0.5 / W[d,k])

            # (10.74)
            EpMuLambda1 <- EpMuLambda1 + log(beta0/(2*pi)) + log_Lambda[d,k] -
                beta0/beta[d,k] - beta0*v[d,k]*((m[d,k] - m0)^2) * W[d,k]
            ...
            EpX <- EpX + Ndk[d,k] * ((c[d]* sum(Elnf[d,,k]*Resp[,k])) + (1-c[d])*sum(lnf_null[,d]*Resp[,k]))
