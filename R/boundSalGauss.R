...
boundSelGauss = function(X, model, prior){
    ...
    for(k in 1:K){
        ...
        for(d in 1:D){
            ...
            Elnf[d,,k] <- log_Lambda[d,k] - 1/beta[d,k] - v[d,k]*W[d,k]*(pollo[,d]^2) - log(2*pi)
    ...
    for(i in 1:D){
        ...
        EqDelta <- EqDelta + (c[d]+o-1)* ElnGammad + (o-c[d]) * ElnOneMinusGammad - lbeta(c[d]+o, 1-c[d]+o)
    }

    EpDelta <- EpDelta - D*lbeta(o,o)

    L = Epz - Eqz + Eppi - Eqpi + EpMuLambda - EqMuLambda + EpX + EpGamma - EqGamma + EpDelta - EqDelta 
